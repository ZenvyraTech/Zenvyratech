...
export const metadata = {
  ...
  alternates: {
    canonical: "https://www.zenvyratech.in/services",
  },
  openGraph: {
    title: "Digital Services | Web Development & SEO Agency",
    description: "Professional web development, technical SEO, and scalable digital solutions for growing businesses.",
                        ...
                        <div className="mt-6">
                          ...
                          <ul className="text-muted-grey space-y-2">
                            {unique.slice(0,3).map(r => (
                              <li key={r.id}><Link href={`/blog/${r.slug}`} className="text-accent hover:text-soft-white">{r.title}</Link></li>
                            ))}
                          </ul>
                        </div>
                      ) : null;
